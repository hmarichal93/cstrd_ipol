...
int save_to_file (unsigned X, unsigned Y, double * mat, FILE* f){
    ...
    for (x = 1; x < (X-1);  x++)
        for(y = 1; y < (Y-1); y++)
            if(y == Y-2)
                ...
                fprintf(f,"%f ", mat[x+y*X]);
}

void save_matrix_to_file(char* file_name,double * M,int X,int Y){
  FILE* file;
...
static void compute_edge_points( double * Ex, double * Ey, double * modG,
                                 double * Gx, double * Gy,double * nonMax, int X, int Y )
{
  int x,y,i;
               ...
               double sigma, double th_h, double th_l,int n )
{
  ...
  double * nonMax   = (double *) xmalloc( X * Y * sizeof(double) );     /* edge_y */
