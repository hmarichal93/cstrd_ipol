"""
Compute metric per error range, per disk
"""
...
from lib.metrics_new_nomenclature import MetricsDataset_over_detection_files
...
def return_list_of_experimental_center( img, cy, cx, error_percentage_range, total_directions= 10):
    """
...
if __name__=="__main__":
    ...
    for idx_images in range(metadata.shape[0]):
        ...
        idx = 0
        for error in error_percentage_range:
            annotations_files_with_same_error = []
            center_error = return_list_of_experimental_center(img_array, cy, cx, [error])
            for center_dt in center_error:
                ...
                labelme_json = load_json(labelme_file)
                cy_f, cx_f = labelme_json['center']
                differencia = lambda x,y: int(np.abs(x-y))
                umbral = 1
                condition = not( differencia(cy_f, cy_dt)<= umbral and differencia(cx_f, cx_dt)<= umbral )
                #print(f"{img_name}_{idx} dif_y {cy_f - cy_dt} dif_x {cx_f - cx_dt}. Status {condition}")
                if condition:
                    continue

                ########################################################################################################
                annotations_files_with_same_error.append(labelme_file)
                idx += 1

            print(f"{img_name}_{idx} Error: {error} len(annotations) {len(annotations_files_with_same_error)}.")

            #compute metrics over this list of files
            output_filename = img_res_dir / f"{img_name}_error_{int(error)}.csv"
            metrics = MetricsDataset_over_detection_files(img_name, annotations_files_with_same_error,output_filename)
            metrics.compute()
